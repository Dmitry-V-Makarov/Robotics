...
// чтобы измерять не скользящую среднюю, а напрямую считывать значение
// достаточно просто объявить #define MA 1
...
void loop() {
  int distance = measure(IR_DIST_SENS); // записываем в distance результат функции measure
  Serial.println(distance);
  analogWrite(LED_PIN, map(distance, 0, 1023, 0, 255));
}

// после loop() задается функция
...
int measure(int pin)
{
  ...
  //функция вернет (return) значение,
